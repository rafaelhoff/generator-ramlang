 ...
 * @returns {Object}
 */
var getParametersForMethod = function(ramlResource, method, methodItem, relativeUri) {
  var result = {
    method: '',
    api: '',
    rawMethod: [],
  ...
  var getAdditionalHeaders = function(item) {
      var contentType = '';
      for (mediaType in item.body) {
      ...
      if (contentType.length > 0) {
        ...
        var resultJSON = resultJSON.replace(/\"/g, '\'');
        return resultJSON;
      } 
  };
  ...
  switch (method) {
    ...
    case 'post':
      ...
      var additionalHeaders = getAdditionalHeaders(methodItem);
      if (additionalHeaders)
        apiParams.push(additionalHeaders);
      break;
    case 'put':
      params.push('entity');
 ...
 */
var generateMethods = function(ramlResource, relativeUri, apiModuleName) {
  ...
  (ramlResource.methods || []).forEach(function(item, index) {
    ...
    var parameters = getParametersForMethod(ramlResource, transformedMethodName, item, relativeUri);
    methodData.push({
 ...
 */
var generateSubResource = function(ramlResource, relativeUri, apiModuleName) {
  ...
  if (subMethodName && hasMethods) {
    return _.template(subResourceTemplateText, {
      resource: {
        ...
        methods: generateMethods(ramlResource, relativeUri, apiModuleName)
      }
    });
  }
 ...
 * @returns {string} The compiled template string.
 */
var recursResources = function(ramlResource, level, relativeUri, apiModuleName) {
  var compiledResource = '';
  var uri = relativeUri + ramlResource.relativeUri;
  if (level !== 0) {
    if (isCollection(ramlResource)) {
      compiledResource += generateSubResource(ramlResource, uri, apiModuleName);
    } else {
      compiledResource += generateMethods(ramlResource, uri, apiModuleName);
    }
  ...
  if (hasResources && compiledResource) {
    ramlResource.resources.forEach(function(resource) {
      ...
      var recursResult = recursResources(resource, level + incrementer, uri, apiModuleName);
 ...
 * @returns {string} The compiled template string
 */
module.exports.generate = function(ramlResource, selectedMediaTypeExtension, apiModuleName) {
  ...
  var compiledTemplate = recursResources(ramlResource, 0, '', apiModuleName);
